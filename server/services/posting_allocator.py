...
def allocate_timetable(
    ...
    # MedComm bonuses: reward GRM selections or two GM blocks when they sit contiguously with MedComm
    medcomm_synergy_bonus_terms = []
    medcomm_synergy_bonus_weight = 1

    if MEDCOMM_codes:
        for resident in residents:
            mcr = resident["mcr"]
            flag = model.NewBoolVar(f"{mcr}_medcomm_synergy_bonus")

            # detect MedComm presence
            hasMedComm = model.NewBoolVar(f"{mcr}_hasMedComm")
            medcomm_selection = sum(selection_flags[mcr][p] for p in MEDCOMM_codes)
            model.Add(medcomm_selection >= 1).OnlyEnforceIf(hasMedComm)
            model.Add(medcomm_selection == 0).OnlyEnforceIf(hasMedComm.Not())

            partner_flags = []

            for institution, medcomm_variants in MEDCOMM_CODES_BY_INSTITUTION.items():
                inst_slug = to_snake_case(f"inst_{institution or 'generic'}")

                medcomm_blocks_by_month = {}
                for b in blocks:
                    medcomm_sum = sum(x[mcr][p][b] for p in medcomm_variants)
                    block_flag = model.NewBoolVar(
                        f"{mcr}_{inst_slug}_medcomm_block_{b}"
                    )
                    model.Add(medcomm_sum == block_flag)
                    medcomm_blocks_by_month[b] = block_flag

                inst_specific_flags = []

                grm_variants = GRM_CODES_BY_INSTITUTION.get(institution, [])
                if grm_variants:
                    grm_blocks_by_month = {}
                    for b in blocks:
                        grm_sum = sum(x[mcr][p][b] for p in grm_variants)
                        block_flag = model.NewBoolVar(
                            f"{mcr}_{inst_slug}_grm_block_{b}"
                        )
                        model.Add(grm_sum == block_flag)
                        grm_blocks_by_month[b] = block_flag

                    grm_contiguous_flag = model.NewBoolVar(
                        f"{mcr}_{inst_slug}_medcomm_grm_contiguous"
                    )
                    adjacency_hits = []
                    for b in blocks:
                        medcomm_block = medcomm_blocks_by_month[b]
                        left_idx = b - 1
                        right_idx = b + 1
                        if left_idx in grm_blocks_by_month:
                            hit = model.NewBoolVar(
                                f"{mcr}_{inst_slug}_medcomm_grm_left_{b}"
                            )
                            model.Add(hit <= medcomm_block)
                            model.Add(hit <= grm_blocks_by_month[left_idx])
                            model.Add(
                                hit >= medcomm_block + grm_blocks_by_month[left_idx] - 1
                            )
                            adjacency_hits.append(hit)
                        if right_idx in grm_blocks_by_month:
                            hit = model.NewBoolVar(
                                f"{mcr}_{inst_slug}_medcomm_grm_right_{b}"
                            )
                            model.Add(hit <= medcomm_block)
                            model.Add(hit <= grm_blocks_by_month[right_idx])
                            model.Add(
                                hit
                                >= medcomm_block + grm_blocks_by_month[right_idx] - 1
                            )
                            adjacency_hits.append(hit)

                    if adjacency_hits:
                        adjacency_sum = sum(adjacency_hits)
                        model.Add(adjacency_sum >= 1).OnlyEnforceIf(grm_contiguous_flag)
                        model.Add(adjacency_sum == 0).OnlyEnforceIf(
                            grm_contiguous_flag.Not()
                        )
                    else:
                        model.Add(grm_contiguous_flag == 0)

                    inst_specific_flags.append(grm_contiguous_flag)

                gm_variants = GM_CODES_BY_INSTITUTION.get(institution, [])
                if gm_variants:
                    gm_blocks_by_month = {}
                    for b in blocks:
                        gm_sum = sum(x[mcr][p][b] for p in gm_variants)
                        block_flag = model.NewBoolVar(f"{mcr}_{inst_slug}_gm_block_{b}")
                        model.Add(gm_sum == block_flag)
                        gm_blocks_by_month[b] = block_flag

                    gm_window_flag = model.NewBoolVar(
                        f"{mcr}_{inst_slug}_medcomm_gm_window"
                    )
                    gm_window_hits = []
                    for start in blocks[:-2]:
                        window_blocks = [start, start + 1, start + 2]
                        gm_count = sum(gm_blocks_by_month[b] for b in window_blocks)
                        medcomm_count = sum(
                            medcomm_blocks_by_month[b] for b in window_blocks
                        )

                        gm_exact_two = model.NewBoolVar(
                            f"{mcr}_{inst_slug}_gm_window_{start}_gm_two"
                        )
                        model.Add(gm_count == 2).OnlyEnforceIf(gm_exact_two)
                        model.Add(gm_count != 2).OnlyEnforceIf(gm_exact_two.Not())

                        medcomm_exact_one = model.NewBoolVar(
                            f"{mcr}_{inst_slug}_gm_window_{start}_mc_one"
                        )
                        model.Add(medcomm_count == 1).OnlyEnforceIf(medcomm_exact_one)
                        model.Add(medcomm_count != 1).OnlyEnforceIf(
                            medcomm_exact_one.Not()
                        )

                        hit = model.NewBoolVar(
                            f"{mcr}_{inst_slug}_gm_window_{start}_hit"
                        )
                        model.Add(hit <= gm_exact_two)
                        model.Add(hit <= medcomm_exact_one)
                        model.Add(hit >= gm_exact_two + medcomm_exact_one - 1)
                        gm_window_hits.append(hit)

                    if gm_window_hits:
                        windows_sum = sum(gm_window_hits)
                        model.Add(windows_sum >= 1).OnlyEnforceIf(gm_window_flag)
                        model.Add(windows_sum == 0).OnlyEnforceIf(gm_window_flag.Not())
                    else:
                        model.Add(gm_window_flag == 0)

                    inst_specific_flags.append(gm_window_flag)

                if inst_specific_flags:
                    if len(inst_specific_flags) == 1:
                        inst_partner_flag = inst_specific_flags[0]
                    else:
                        inst_partner_flag = model.NewBoolVar(
                            f"{mcr}_{inst_slug}_medcomm_partner"
                        )
                        for subflag in inst_specific_flags:
                            model.Add(inst_partner_flag >= subflag)
                        model.Add(inst_partner_flag <= sum(inst_specific_flags))
                    partner_flags.append(inst_partner_flag)

            if partner_flags:
                if len(partner_flags) == 1:
                    partner_condition = partner_flags[0]
                else:
                    partner_condition = model.NewBoolVar(
                        f"{mcr}_medcomm_partner_condition"
                    )
                    for partner in partner_flags:
                        model.Add(partner_condition >= partner)
                    model.Add(partner_condition <= sum(partner_flags))

                model.Add(flag <= hasMedComm)
                model.Add(flag <= partner_condition)
                model.Add(flag >= hasMedComm + partner_condition - 1)
                model.Add(flag == 0).OnlyEnforceIf(hasMedComm.Not())
            else:
                model.Add(flag == 0)

            medcomm_synergy_bonus_terms.append(medcomm_synergy_bonus_weight * flag)
    ...
    model.Maximize(
        ...
        + sum(medcomm_synergy_bonus_terms)  # static, 1
